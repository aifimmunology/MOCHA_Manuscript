...
plot_coaccessibility <- function(zi_mat, index, peak_mat,fname, logT = TRUE, returnPlot = FALSE, addLabel=F){
    
    if(logT){
        df = data.frame(
            Values_X = 2^(as.numeric(peak_mat[zi_mat$Tile1[index],]))-1,
            Values_Y = 2^(as.numeric(peak_mat[zi_mat$Tile2[index],]))-1
            )
    }else{
         df = data.frame(
            Values_X = as.numeric(peak_mat[zi_mat$Tile1[index], ]),
            Values_Y = as.numeric(peak_mat[zi_mat$Tile2[index], ])
            )
    }
    ...
    zi_spear_cor = MOCHA:::weightedZISpearman(df[,1], df[,2])
    ...
    p <- ggplot(df,
        ...
        ggtitle(paste("Co-Accessibility"))+xlim(0,x2+2)+ylim(0,y2+2)+
            xlab(zi_mat$Tile1[index])+ylab(zi_mat$Tile2[index])+
            ...
            geom_text(  x = c(3), y = c(6),
                  label=paste('S=',
                               round(spear_cor,2)),
                                       size=10,
                             col='brown')+
            geom_smooth(method='lm', se=FALSE,size=2,col='brown',linetype='dashed')+
