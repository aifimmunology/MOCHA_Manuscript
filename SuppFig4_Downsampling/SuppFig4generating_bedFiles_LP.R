...
lp_ArchRProject <- addCellColData(lp_ArchRProject, 
                             data =cellType, 
                             name = "new_cellType", 
                             cells = getCellNames(lp_ArchRProject))
...
#celltypes <- c('Plasmablast','B naive','NK_CD56bright','CD14 Mono')
celltypes <- c('B naive', 'CD16 Mono', 'CD8 TEM')

callPeaks = TRUE 

setwd('/home/jupyter/longPilotValidation')
...
for(cell in celltypes){
    ...
    cellType_dir <- paste('/home/jupyter/longPilotValidation/ManuscriptData/',
                     cell, 
                     sep='')    
