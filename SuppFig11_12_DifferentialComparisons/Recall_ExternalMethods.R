...
allRecall <- rbind(data.frame(
      ...
      allRecall)
allRecall <- dplyr::filter(allRecall, SampleNumber > 29)

pdf('Method_Recall.pdf')
ggplot(allRecall, aes(x = SampleNumber, y = Recall, color = Method, group = Method)) + 
    geom_smooth() + geom_jitter()+theme_bw() + xlab('Number of Samples') + ylab('Recall') + scale_x_continuous(breaks = seq(30,39, by = 2))+
