...
hist(ZI_Corr2$pValues)
hist(Norm_Corr$pValues)
hist(ZI_Corr2$Correlation)
hist(Norm_Corr$Correlation)
hist(Norm_Corr$Correlation)
dev.off()
...
foreGround_Norm$pValues = pValues_N

### Now let's test against a completely random background
randoBack <- data.frame(Tile1 = sample(rownames(accMat), 1000), 
                       Tile2 = sample(rownames(accMat), 1000))
subAccMat <- accMat[unique(c(randoBack[,1],randoBack[,2])),]

cl <- paralle::makeCluster(55)
message(paste('Generating Background correlations for all', length(tile1), 'pairs', sep = " "))
parallel::clusterExport(cl, varlist = c('subAccMat', "randoBack"), envir = environment())

randoBackCorr <- runCoAccessibility(subAccMat, randoBack , ZI= TRUE, verbose = TRUE, cl)
randoBackCorr_N <- runCoAccessibility(subAccMat, randoBack , ZI= FALSE, verbose = TRUE, cl)

hist(randoBackCorr$Correlation)
hist(randoBackCorr_N$Correlation)

pValues_rZ <- lapply(1:1000, function(x){
    
    cor1 <- foreGround$Correlation[x]
    
    if(cor1 >= 0){
        
        sum(cor1 < randoBackCorr$Correlation, na.rm = TRUE)/length(randoBackCorr$Correlation)
...
pValues_rN <- lapply(1:1000, function(x){
    
    cor1 <- foreGround_Norm$Correlation[x]
    
    if(cor1 >= 0){
        
        sum(cor1 < randoBackCorr_N$Correlation, na.rm = TRUE)/length(randoBackCorr$Correlation)
...
saveRDS(foreGround, 'CoAccessibility_ForeGround.rds')
saveRDS(foreGround_Norm, 'CoAccessibility_ForeGround_Norm.rds')

foreGround <- readRDS('CoAccessibility_ForeGround.rds')
foreGround_Norm <- readRDS('CoAccessibility_ForeGround_Norm.rds')

ZI_randoBack <- testCoAccessibility2(STM, fTiledLinks$Tile1, fTiledLinks$Tile2, numCores = 55, ZI = TRUE, 
                     backNumber = 2000,verbose = TRUE)
Norm_randoBack <- testCoAccessibility2(STM, fTiledLinks$Tile1, fTiledLinks$Tile2, numCores = 55, ZI = FALSE, 
                     backNumber = 2000,verbose = TRUE)

#Background set of 10,000
backgroundRows <- ! rownames(fullObj) %in% c(fTiledLinks$Tile1, fTiledLinks$Tile2)

randoBack <- data.frame(Tile1 = sample(rownames(accMat)[backgroundRows], 10000), 
                       Tile2 = sample(rownames(accMat)[backgroundRows], 10000))
subAccMat <- accMat[unique(c(randoBack[,1],randoBack[,2])),]
...
randoBackCorr <- runCoAccessibility(subAccMat, randoBack , ZI= TRUE, verbose = TRUE, cl)
randoBackCorr_N <- runCoAccessibility(subAccMat, randoBack , ZI= FALSE, verbose = TRUE, cl)
saveRDS(randoBackCorr, 'CoAcc_RandoBackground_large_ZI.rds')
saveRDS(randoBackCorr_N, 'CoAcc_RandoBackground_large_Norm.rds')
...
pdf('CoAcc_RandomBackground.pdf')
par(mfrow=c(3,2))
hist(foreGround$Correlation)
hist(foreGround_Norm$Correlation)
...
########################################## Run on all links
combPairs <- data.frame(fTiledLinks$Tile1, fTiledLinks$Tile2)
subAccMat <- accMat[unique(c(combPairs[,1], combPairs[,2])),]

all(combPairs[,1] %in% rownames(subAccMat))
all(combPairs[,2] %in% rownames(subAccMat))

cl <- makeCluster(55)
parallel::clusterExport(cl, varlist = c('subAccMat', "combPairs"), envir = environment())
foreGround <- runCoAccessibility(subAccMat, combPairs, ZI = TRUE, verbose = TRUE, cl)
saveRDS(foreGround, 'CoAccessibility_ForeGround_All.rds')
rm(foreGround)
foreGround_Norm <- runCoAccessibility(subAccMat, combPairs, ZI = FALSE, verbose = TRUE, cl)
rm(foreGround_Norm)
saveRDS(foreGround_Norm, 'CoAccessibility_ForeGround_Norm_All.rds')

randoBack <- data.frame(Tile1 = sample(rownames(accMat)[backgroundRows], 100000), 
                       Tile2 = sample(rownames(accMat)[backgroundRows], 100000))
subAccMat <- accMat[unique(c(randoBack[,1],randoBack[,2])),]
cl <- parallel::makeCluster(55)
parallel::clusterExport(cl, varlist = c('subAccMat', "randoBack"), envir = environment())
...
cl <- parallel::makeCluster(55)
parallel::clusterExport(cl, varlist = c('foreGround', "randoBackCorr", "randoBackCorr_N", "foreGround_Norm"), 
                        envir = environment())


foreGround$pValues <- calculatePValues(foreGround, randoBackCorr, cl)
foreGround_Norm$pValues <- calculatePValues(foreGround_Norm, randoBackCorr_N, cl)
foreGround$FDR <- p.adjust(foreGround$pValues, method = 'fdr')
foreGround_Norm$FDR <- p.adjust(foreGround_Norm$pValues, method = 'fdr')
saveRDS(foreGround, 'CoAccessibility_ForeGround_All.rds')
saveRDS(foreGround_Norm, 'CoAccessibility_ForeGround_Norm_All.rds')

ks.test(foreGround$Correlation, randoBackCorr$Correlation, alternative = "less")
ks.test(foreGround_Norm$Correlation, randoBackCorr_N$Correlation, alternative = "less")

ZI_Spearman <- rbind(foreGround[,c(1:3)], randoBackCorr)
ZI_Spearman$Type = c(rep('Foreground', dim(foreGround)[1]), 
                     rep('Background', dim(randoBackCorr)[1]))
write.csv(ZI_Spearman, 'CoAcc_ZI_Spearman.csv')

Spearman <- rbind(foreGround_Norm[,c(1:3)], randoBackCorr_N)
Spearman$Type = c(rep('Foreground', dim(foreGround_Norm)[1]), 
                     rep('Background', dim(randoBackCorr_N)[1]))
write.csv(Spearman, 'CoAcc_Spearman.csv')
