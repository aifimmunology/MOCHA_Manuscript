...
summarize_cell <- function(cell){
    ...
    #####################################################################
    ### Load MOCHA tiles
    setwd('/home/jupyter/covid/scMACS_manuscript_analyses/ManuscriptData')
    setwd(cell)
    setwd('bulk')
    setwd('MOCHA')
...
a = ggplot(results_tiles,
    ...
    ylab('Tile Counts')+scale_col_MOCHA()
...
b = ggplot(results_ctcf,
       aes(x=CellCounts,
        ...
        theme_minimal()+
    facet_wrap(~Cell, ncol=3, scales='free')+
    ylab('CTCF Counts')+scale_col_MOCHA()
### get TSS counts 
...
c = ggplot(results_tss,
       aes(x=CellCounts,
        ...
        theme_minimal()+
    facet_wrap(~Cell, ncol=3, scales='free')+
    ylab('TSS Counts')+scale_col_MOCHA()

### 
require(ggpubr)
ggarrange(a,b,c, ncol=1,
                 common.legend=T)
ggsave(file='covid_downsampling.pdf')
