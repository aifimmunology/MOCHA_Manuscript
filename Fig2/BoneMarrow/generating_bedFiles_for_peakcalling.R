...
for(cell in celltypes){
    ...
    cellType_dir <- paste('/home/jupyter/BoneMarrowValidation/data/',
                     cell, 
                     sep='')    
    ...
    peak_lists <- mclapply(1:length(subset_ArchR_projects),
                           ...
                           try(callPeaks_by_population(ArchRProj=subset_ArchR_projects[[x]],
                                            cellSubsets=cell,
                                            cellCol_label_name='new_cellType',
                                            returnAllPeaks=TRUE,
                                            numCores=1,
                                            totalFrags=normalization_factors[x],
                                            fragsList =subsetFragsList[[x]]
                           )),
                          mc.cores=5
    ...
    ### create directory
    dir.create('scMACS_peaks')
    setwd('scMACS_peaks/')

    ### filter any possible errors 
    idx <- which(sapply(peak_lists, function(x) class(x))=='list')
    ...
    mclapply(idx,
             function(x)
                  saveRDS(peak_lists[[x]], 
                          file=paste(fname[x],'.RDS', sep='')
                          ),
             mc.cores=10
             )
                        
    print('finished calling scMACS peak on population')        
